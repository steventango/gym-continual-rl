...
def test_puddle_env_custom_task_0():
    env = gym.make("gym_continual_rl/Puddle-v0", render_mode="rgb_array", env_setups=CUSTOM_ENV_SETUPS, task=0)
    _ = env.reset(seed=0)
...
def test_puddle_env_custom_task_1():
    env = gym.make("gym_continual_rl/Puddle-v0", render_mode="rgb_array", env_setups=CUSTOM_ENV_SETUPS, task=1)
    _ = env.reset(seed=0)
