...
def test_jbw_env_period_0_25():
    ...
    obs, reward, terminated, truncated, info = env.step(RIGHT)
    np.testing.assert_almost_equal(reward, 1.0)
    assert not terminated
    assert not truncated
...
def test_jbw_env_render():
    env = gym.make("gym_continual_rl/JBW-v2", render_mode="rgb_array")
    env.reset(seed=0)
    frame = env.render()
...
def test_jbw_env_render_zoomed_out():
    env = gym.make("gym_continual_rl/JBW-v2", render_mode="rgb_array")
    env.reset(seed=0)
    env.unwrapped._painter = MapVisualizer(
        env.unwrapped._sim, env.unwrapped.sim_config, bottom_left=(-512, -512), top_right=(512, 512)
    )
